...
const UserBurdenCalculator: FC<UserBurdenCalculatorProps> = ({
	...
	useEffect(() => {
		...
		const latex = {
			steerableSize: `
				...
				\\text{Number of elements in X:} v_tag i_tag (N_x) ${inputs.N_x}\n
				\\text{Number of elements in Y:} v_tag i_tag (N_y) ${inputs.N_y}\n
				\\text{Element spacing in X } (\\lambda)\\text{:} v_tag i_tag (lambda_x) ${inputs.lambda_x}\n
				\\text{Element spacing in Y } (\\lambda)\\text{:} v_tag i_tag (lambda_y) ${inputs.lambda_y}\n
				\\text{Ideal Element Directivity (dB):} v_tag i_tag (G_IED) ${inputs.G_IED}\n
				...
				\\text{EIRP at OP1dB (dBW):} v_tag${addComma(results.EIRP.toFixed(1))}\n
				\\textbf{Array Size:}\n
				\\text{Size in X (m):} v_tag${addComma(results.size_x.toFixed(2))}\n
				\\text{Size in Y (m):} v_tag${addComma(results.size_y.toFixed(2))}\n
			`,
			patchSize: `
				\\text{Dielectric Constant} (\\epsilon_{r}) v_tag i_tag (epsilon_r) ${inputs.epsilon_r}\n
				\\text{Antenna Height (mm)} v_tag i_tag (ant_height) ${inputs.ant_height}\n
				\\text{Frequency (GHz)} v_tag${addComma((rtnLinkFreqMHz / 1000).toFixed(3))}\n
				\\text{c (m/s)} v_tag ${(3e8).toExponential()}\n
				\\text{Width (mm)} v_tag${addComma(results.width.toFixed(2))}\n
				\\text{Length (mm)} v_tag${addComma(results.length.toFixed(2))}\n
				\\text{Size = Length (mm) * Width (mm)} v_tag${addComma(results.size.toFixed(2))}\n
			`
		};
	...
	const row = (
		...
		return (
			<Grid container key={`div_${index}`}>
				<Grid item md={6} className={`${myCLasses.italicFont} text-left p-2`} style={{
					display: 'flex',
					fontStyle: 'italic',
					alignItems: 'center',
				}}>
	...
	return (
		...
		>
			...
			<Grid className='px-2 py-2' style={{
				boxShadow: '0 4px 14px rgba(0,0,0,10%)',
				height: '100%',
				display: 'flex',
